...
classDiagram
    ...
    class TransferQueueDataPlane {
        ...
        +put_rollout_batch(scope, partition_id, samples)
        +claim_reward_batch(scope, partition_id, batch_size)
        +append_rewards(metadata, rewards)
        +claim_reward_ready_groups(scope, partition_id, num_generations, max_groups)
        +append_advantages(group_metadata, advantages)
        +build_streaming_dataset(scope, partition_id, required_fields, batch_size, sampler)
        +read_train_batch(metadata)
        +ack_train_batch(metadata)
        +mark_trained(metadata)
        +clear_partition(scope, partition_id)
    }
    ...
    class StalenessManager {
        ...
        +float sleep_seconds
        +get_rollout_capacity(metadata, policy_version, scope) RolloutCapacity
...
sequenceDiagram
    ...
    loop pipeline run loop
        P->>Q: get_metadata()
        Q-->>P: QueueMetadata
        P->>S: get_rollout_capacity(metadata, policy_version, scope)
        S-->>P: RolloutCapacity
        ...
        alt train_k done
            ...
            P->>C: sync_weights(scope, adapter_name, train_k)
            C->>V: receive_weights(adapter_name, policy_version)
            P->>Q: clear_partition(scope, train_k)
        end
    end
...
scope_max_live_partitions:
  单个 (tenant_id, training_run_id, adapter_name) 的 live train_k 上限。
  默认等于 max_staleness + 1。
```

`StalenessManager` 使用 scope 内的 live partitions 计算 capacity；`TransferQueueDataPlane` 负责执行全局和 scope 级容量保护，防止一个 tenant / adapter 占满整个 TQ。
...
```text
一个 train_k 对应一个 policy_version。
Trainer batch 只从同一个 train_k / policy_version 读取。
...
所有 `claim / read / ack / append / clear` 必须携带 `TrainingScope` 过滤条件：

```text
claim(scope, partition_id=train_k, task_name=reward, ...)
read(scope, partition_id=train_k, task_name=train, ...)
clear_partition(scope, partition_id=train_k)
```

禁止跨 scope claim 数据；这是 multi-tenant 隔离和 multi-LoRA 训练正确性的硬约束。
...
第一版不支持一个 trainer batch 混合多个 `adapter_name` 或多个 `policy_version`。后续如果要支持 mixed version batch，需要在 advantage、loss mask、importance correction 和权重同步协议里额外设计。
...
```text
active_partitions = metadata.active_partitions  # 已按 TrainingScope 过滤
...
TrainerWorker 完成 train_k
  -> BaseRLPipeline.sync_weights(scope, adapter_name, train_k)
  -> CheckpointEngineManager
  -> vLLMSampler.receive_weights(adapter_name, policy_version)
  -> TransferQueueDataPlane.clear_partition(scope, train_k)
```

partition 被 clear 后，下一次 `get_metadata(scope)` 会反映该 scope 内 active partition 数减少，`StalenessManager` 计算出的 capacity 自然变大。
...
partial rollout 状态必须保存 `TrainingScope`。中断续跑时，只允许把 partial state 交回同一个 `tenant_id / training_run_id / adapter_name`，不能跨 tenant 或跨 adapter 续跑。
...
checkpoint:
  ...
  sync_scope: adapter                # multi-LoRA 场景只同步当前 adapter_name
...
- 固化 `TrainingScope = tenant_id / training_run_id / adapter_name`，所有 TQ 操作必须按 scope 过滤。
...
class TransferQueueDataPlane:
    ...
    def put_rollout_batch(self, scope: TrainingScope, partition_id: str, samples: list[dict]): ...
    def claim_reward_batch(self, scope: TrainingScope, partition_id: str, batch_size: int): ...
    def append_rewards(self, metadata, rewards): ...
    def claim_reward_ready_groups(self, scope: TrainingScope, partition_id: str, num_generations: int, max_groups: int): ...
    def append_advantages(self, group_metadata, advantages): ...
    def build_streaming_dataset(self, scope: TrainingScope, partition_id: str, required_fields: list[str], batch_size: int, sampler): ...
    def mark_trained(self, metadata): ...
    def clear_partition(self, scope: TrainingScope, partition_id: str): ...
```
...
```python
class StalenessManager:
    def get_rollout_capacity(self, metadata: QueueMetadata, policy_version: int, scope: TrainingScope) -> RolloutCapacity: ...
...
- `train_k` 内 batch 来自同一个 `policy_version`。
