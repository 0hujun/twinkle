...
classDiagram
    ...
    class AsyncRollouter {
        ...
        +Queue pending_queue
        +Dict active_tasks
        +Queue finished_tasks
        +Queue completed_results
        +int max_concurrency
        +int max_submit_groups
...
- 实现 `AsyncRollouter` 的 `pending_queue / active_tasks / completed_results`。
...
- rollout task 完成后立即将 ready prompt groups 写入 `TransferQueueDataPlane.put_rollout_batch(train_k)`，不等待其他 active task。
